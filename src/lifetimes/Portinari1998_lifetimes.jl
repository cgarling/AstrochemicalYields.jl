...
120 3.11E+06 3.23E+06 3.32E+06 3.23E+06 3.11E+06]
    M = SVector{30, Float64}(data[:,1])
    # itp_vec = [SVector{5, Float64}(data[i,2:end]) for i in axes(data,1)]
    # itp = interpolate((Z,), table_vec, Gridded(Linear()))
    itp = interpolate((M, Z), data[:, 2:end] ./ 1e9, Gridded(Linear()))
    itp = extrapolate(itp, bounds)
    return Portinari1998Lifetimes(itp)
end